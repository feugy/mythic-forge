...
class _ImagesService
  ...
  # @option meta ext [String] the image extension
  # @option meta width [Number] image's width in pixel
  # @option meta height [Number] image's height in pixel
  # @param imageData [Buffer] image binary data
  ...
  save: (model, meta, imageData, args...) =>
    switch args.length
      ...
      when 2 
        ...
        throw new Error "idx argument #{suffix} isn't a positive number" unless _.isNumber(suffix) and suffix >= 0
        existing = model.get('images')[suffix]?.file
      else throw new Error "save must be called with arguments (model, imageData, [idx], callback)"
    ...
    proceed = (err) =>
      ...
      fs.writeFile path.join(imagesPath, fileName), imageData, (err) =>
        return callback "failed to save image #{suffix} on model #{model._id}: #{err}" if err?
  ...
  # @option callback model [Object] the saved model
  #
  # @overload save(model, callback)
  #   Removes the type image of the given model
  #
  # @overload save(model, idx, callback)
  ...
  remove: (model, args...) =>
    switch args.length
      ...
      when 2 
        ...
        throw new Error "idx argument #{suffix} isn't a positive number" unless _.isNumber(suffix) and suffix >= 0
        existing = model.get('images')[suffix]?.file
      else throw new Error "remove must be called with arguments (model, [idx], callback)"
